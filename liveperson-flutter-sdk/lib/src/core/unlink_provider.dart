...
class UnlinkProvider {
  ...
  /// (the ID of the provider to unlink) and makes a request to liveperson to unlink the provider.
  ///
  /// Parameters:
  ...
  ///
  /// Throws:
  /// - [livepersonAuthException] if the request to unlink the provider fails.
  Future<User> unlinkProvider(String? idToken, String providerId) async {
    try {
      ...
      // Perform the request to liveperson to unlink the provider
      final response = await auth.performRequest(
        'update', // liveperson endpoint to update user info
        {
