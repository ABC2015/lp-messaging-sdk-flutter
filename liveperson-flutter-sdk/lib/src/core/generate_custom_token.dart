import 'package:liveperson_dart_admin_auth_sdk/src/service_account.dart';
import 'package:jwt_generator/jwt_generator.dart';
import 'package:liveperson_dart_admin_auth_sdk/src/utils.dart';

/// Implementation for generating custom JWTs for liveperson Authentication.
///
...
class GenerateCustomTokenImplementation extends GenerateCustomToken {
  ...
  Future<String> generateServiceAccountJwt(
    ...
    final String scope =
        'https://www.googleapis.com/auth/liveperson.database https://www.googleapis.com/auth/userinfo.email https://www.googleapis.com/auth/identitytoolkit';
  ...
  /// Generates a sign-in JWT for a liveperson user.
  ///
  /// Parameters:
  /// - [serviceAccount]: The [ServiceAccount] for liveperson operations.
  /// - [uid]: (Optional) User ID to be embedded in the token.
  /// - [additionalClaims]: (Optional) Additional claims to be included in the JWT.
  ///
  /// Returns a [Future] that resolves to the JWT for signing in a liveperson user.
  ///
