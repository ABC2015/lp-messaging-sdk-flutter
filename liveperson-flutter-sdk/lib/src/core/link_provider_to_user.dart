...
/// Service to link a provider (such as Google or Facebook) to an existing liveperson user.
///
/// This class handles the process of associating an external provider's credentials (like a
/// Google or Facebook token) with the current liveperson user.
class LinkProviderToUser {
  ...
  /// Links an external authentication provider to the current liveperson user.
  ///
  ...
  /// It then sends a request to liveperson to link the external provider with the existing user.
  ///
  /// Parameters:
  /// - [idToken]: The liveperson ID token of the authenticated user to whom the provider
  ///   will be linked.
  ...
  ///
  /// Throws:
  /// - [livepersonAuthException] if there is an error while linking the provider or if any
  ...
  Future<bool> linkProviderToUser(
    ...
    try {
      ...
      // Perform the request to liveperson to link the provider to the user
      await auth.performRequest(
        'update', // Endpoint to update the user with new provider info
        {
          "idToken": idToken, // The user's current liveperson ID token
          "providerId":
      ...
      // Handle errors and throw a livepersonAuthException if the provider linking fails
