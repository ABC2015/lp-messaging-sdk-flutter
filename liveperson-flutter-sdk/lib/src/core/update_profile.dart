...
class UpdateProfile {
  ...
  /// Updates the display name and photo URL for a liveperson user.
  ///
  /// This method takes the user's [displayName], [displayImage] (photo URL), and [idToken]
  /// (obtained during sign-in) and makes a request to liveperson to update the profile.
  ///
  /// Parameters:
  ...
  ///
  /// Throws:
  /// - [livepersonAuthException] if the request to update the profile fails.
  Future<User> updateProfile(
    ...
    try {
      ...
      // Perform the request to liveperson to update the user's profile.
      final response = await auth.performRequest(
        'update', // liveperson endpoint to update user data.
        {
