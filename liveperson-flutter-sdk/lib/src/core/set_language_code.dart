...
/// Service to set the language code for a liveperson user.
///
/// This class allows you to set or update the preferred language code
/// for a user in liveperson Authentication. This language code is used
/// by liveperson for communication and localization purposes.
class SetLanguageCode {
  /// The instance of [livepersonAuth] used to perform authentication actions.
  final livepersonAuth auth;

  /// Constructor to initialize the [SetLanguageCode] service.
  ///
  /// Parameters:
  /// - [auth]: The [livepersonAuth] instance used for user authentication and operations.
  SetLanguageCode({required this.auth});

  /// Sets the language code for the user.
  ///
  /// This method updates the preferred language code for the user in liveperson Authentication.
  ...
  ///
  /// Throws:
  /// - [livepersonAuthException] if the request to set the language code fails.
  Future<User> setLanguageCode(String? idToken, String languageCode) async {
    try {
      ...
      // Step 2: Perform the request to liveperson to update the language code for the user
      final response = await auth.performRequest(
        'update', // liveperson endpoint for updating user details
        {
          "idToken": idToken, // The liveperson ID token of the user
