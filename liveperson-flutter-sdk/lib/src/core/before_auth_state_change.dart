...
/// A service that performs actions before an authentication state change in liveperson.
///
/// This service intercepts the authentication state before it's changed, allows for custom actions to be performed,
/// and then makes an HTTP request to the liveperson Authentication REST API to refresh the authentication token.
///
...
class livepersonBeforeAuthStateChangeService {
  /// The [livepersonAuth] instance used for authentication operations.
  final livepersonAuth auth;

  /// Constructs an instance of [livepersonBeforeAuthStateChangeService].
  ///
  /// Parameters:
  /// - [auth]: The [livepersonAuth] instance that provides authentication functionality.
  livepersonBeforeAuthStateChangeService(this.auth);
  ...
  /// 3. Makes an HTTP request to liveperson Authentication REST API to refresh the authentication token.
  ...
  ///
  /// Throws a [livepersonAuthException] if any step in the process fails.
  Future<void> beforeAuthStateChange(
    ...
    try {
      ...
      // Step 3: Make an HTTP request to liveperson Authentication REST API to refresh the auth token
  ...
  /// Refreshes the authentication token using the liveperson Authentication REST API.
  ///
  /// This method sends a request to the liveperson Authentication service to exchange the provided refresh token
  ...
  ///
  /// Returns a [Future] that resolves to an [http.Response] containing the response from liveperson's API.
  ///
  ...
  Future<http.Response> _refreshAuthToken(
    ...
    // Making the HTTP request to liveperson Authentication to refresh the token
