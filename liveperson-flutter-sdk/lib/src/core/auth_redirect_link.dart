...
/// A service class for handling sign-in with redirect functionality using liveperson.
///
/// This class uses liveperson's REST API to sign in with an identity provider (IDP)
/// by sending an ID token and provider ID, and then updating the liveperson Auth
/// user instance accordingly.
class SignInWithRedirectService {
  /// The liveperson authentication instance for managing user authentication.
  final livepersonAuth auth;

  /// Creates a new instance of [SignInWithRedirectService].
  ///
  /// The [auth] parameter is required to interact with the liveperson Authentication API.
  SignInWithRedirectService({required this.auth});

  /// Signs in a user with a redirect URI, ID token, and provider ID.
  ///
  /// This method sends an HTTP POST request to liveperson's REST API, using the provided
  ...
  ///
  /// [redirectUri]: The URI to which liveperson should redirect after successful authentication.
  ...
  Future<UserCredential?> signInWithRedirect(
    ...
    try {
      // liveperson REST API URL for signing in with an identity provider
      String url =
      ...
      if (response.statusCode == 200) {
        ...
        // Update current user in liveperson authentication
        auth.updateCurrentUser(userCredential.user);
        log("Current user ID token: ${userCredential.user.idToken}");

        // Set current user in liveperson app instance
