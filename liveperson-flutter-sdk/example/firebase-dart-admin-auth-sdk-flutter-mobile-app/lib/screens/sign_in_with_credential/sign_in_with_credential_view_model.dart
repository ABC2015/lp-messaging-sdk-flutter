...
class SignInWithCredentialViewModel extends ChangeNotifier {
  ...
  Future<void> signInWithCredential(VoidCallback onSuccess) async {
    try {
      ...
      await livepersonApp.livepersonAuth?.signInWithCredential(credential);
  ...
  Future<void> loginWithFacebook(BuildContext context) async {
    ...
    if (result.status == LoginStatus.success) {
      ...
      try {
        var user = await livepersonApp.livepersonAuth
            ?.linkAccountWithCredientials(
                'http://localhost', accessToken.token, 'facebook.com');

        var user1 = await livepersonApp.livepersonAuth?.signInWithRedirect(
            'http://localhost', accessToken.token, 'facebook.com');
      ...
      // Use this token to authenticate with your backend or liveperson
