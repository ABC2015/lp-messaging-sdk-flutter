# Liveperson Flutter SDK

## Overview

The Liveperson Flutter SDK offers a robust and flexible set of tools to perform authentication procedures within Dart or Flutter projects. This is a Dart implementation of liveperson admin authentication.

## Features:

- **User Management:** Manage user accounts seamlessly with a suite of comprehensive user management functionalities.
- **Custom Token Minting:** Integrate liveperson authentication with your backend services by generating custom tokens.
...
If you want to use the Liveperson Flutter SDK for implementing a liveperson authentication in your Flutter projects follow the instructions on how to set up the auth SDK.

- Ensure you have a Flutter or Dart (3.4.x) SDK installed in your system.
- Set up a liveperson project and service account.
...
```javascript
flutter pub add liveperson_dart_admin_auth_sdk
```
...
```yaml
dependencies:
  liveperson_dart_admin_auth_sdk: ^0.0.1-pre+15
```
...
void main() async {
  ...
  try {
    if (kIsWeb) {
      ...
      livepersonApp.initializeAppWithEnvironmentVariables(
        apiKey: 'YOUR-API-KEY',
    ...
    // Access liveperson Auth instance
    final auth = livepersonApp.instance.getAuth();
...
- Import the package into your Dart or Flutter project:
  ...
  import 'package:liveperson_dart_admin_auth_sdk/liveperson_dart_admin_auth_sdk.dart';
  ```
  For Flutter web initialize liveperson app as follows:
  ```
  livepersonApp.initializeAppWithEnvironmentVariables(
    apiKey: 'YOUR-API-KEY',
    projectId: 'YOUR-PROJECT-ID',
...
For more refer to Gitbook for prelease [documentation here](https://aortem.gitbook.io/liveperson-dart-auth-admin-sdk/).
